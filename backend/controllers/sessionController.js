...
import {
  ...
  generateCoachFeedback as generateCoachFeedbackFromAI,
...
const generateSessionSummaryHandler = asyncHandler(async (req, res) => {
  const session = await Session.findOne({
    _id: req.params.id,
    user: req.user._id,
  }).populate("client", "name email");
  ...
  if (session) {
    ...
    try {
      const summary = await generateAISessionSummary(session);
...
const generateCoachFeedback = asyncHandler(async (req, res) => {
  ...
  if (session) {
    ...
    try {
      const feedback = await generateCoachFeedbackFromAI(session);
...
export {
  ...
  updateSession,
  addAISuggestions,
  generateSessionSummaryHandler as generateSessionSummary,
  sendSessionSummary,
