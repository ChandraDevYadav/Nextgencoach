...
import SkillBuilderSession from '../models/SkillBuilderSession.js';
import { generateSkillBuilderFeedback } from '../services/aiService.js';

const getSkillBuilderSessions = asyncHandler(async (req, res) => {
  const sessions = await SkillBuilderSession.find({ user: req.user._id })
    .sort({ createdAt: -1 });
  res.json(sessions);
});
...
const generateSkillBuilderFeedback = asyncHandler(async (req, res) => {
  ...
  if (session) {
    ...
    try {
      const feedback = await generateSkillBuilderFeedback(session);
